...
bool isWinner(bool isPlayer1, int lastCol) {
  ...
  //then check that piece and three below are same
    ...
    //changes made by me here
  //Check if current player has 4 horizontal tiles in a row
  hcounter = 0; //counter for  4 consecutive 'O' or 'X'
  for (int r = 0; r < ROWS; r++){
      for (int c = 0; c < COLS; c++){
          if (board[c][r] == currTile){
              ...
              //counter resets to 0 if position is not currTile
              hcounter = 0;
  ...
  //couldnt figure out a way to do this diagonally
  //attempt for diagonal:
  dcounter = 0;
  for (int r = ROWS-1;  r >=0 ; r--){
